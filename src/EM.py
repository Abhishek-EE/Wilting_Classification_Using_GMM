from src.Distributions.gaussPDF import gaussPDF
import numpy as np
import sys
def EM(Data, Priors0, Mu0, Sigma0,iterations = 100):
    realmax = sys.float_info[0]
    realmin = sys.float_info[3]
    loglik_threshold = 1e-10
    nbVar, nbData = np.shape(Data) #nbvar = 1200, nbData = 1000
    nbStates = np.size(Priors0) # nbStates = 10
    loglik_old = -realmax #Start from top making an initial comparision ease
    nbStep = 0 #step size
    Mu = Mu0 #reassigning mu mu = (1200,10)
    Sigma = Sigma0 # (1200,1200,10)
    Priors = Priors0 #(1,10)

    Pix = np.ndarray(shape = (nbStates, nbData)) #(10,1000)
    Pxi = np.ndarray(shape = (nbData, nbStates)) #(1000,10)
    for iterate in range(iterations):
        for i in range (0,nbStates):#runs for 10 times
            Pxi[:,i] = gaussPDF(Data,Mu[:,i],Sigma[:,:,i])#calculates pxi for 
